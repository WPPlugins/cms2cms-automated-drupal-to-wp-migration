...
if ($('.cms2cms-drupal-container').length > 0) {
    ...
    var init = function () {
        ...
        if ($('.active').hasClass('step-sign')) {
            ...
            $(document).on('click', '#verifySource_drupal', function (event) {
        ...
        $('#auth_submit').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.key) {
                        saveUser({
                            action: 'cms2cms_drupal_save_options',
                            accessKey: data.key,
    ...
    var toggleSignIN = function () {
        var cms2cmsBlock = $('#cms2cms_drupal_accordeon');
        var signInTabs = cms2cmsBlock.find('a.nav-tab');

        signInTabs.on('click', function (e) {
            if (!$(this).hasClass('cms2cms-drupal-real-link')) {
                e.preventDefault();
                var activeClass = 'nav-tab-active';
                if (!$(this).hasClass(activeClass)) {
                    signInTabs.removeClass(activeClass);
                    $(this).addClass(activeClass);
                    $(this).closest('.cms2cms_drupal_accordeon_item').find('form').attr('action', $(this).attr('href'));
                }
            }
        });

        var formContent = $('.cms2cms_drupal_accordeon_item_register').find('.center-content'),
            button = $('.login-reg');
        ...
        button.on('click', function () {
            ...
            $('.cms2cms-drupal-real-link').fadeToggle();
            button.parent('p.account-register').toggle();
            button.parent('p.account-login').toggle();
            $('.cms2cms-drupal-error span').parent().fadeOut();
    ...
    var checkDoneTab = function () {
        ...
        if ($('.cms2cms-drupal-container').hasClass('cms2cms_drupal_is_activated')) {
    ...
    var validateSource = function () {
        ...
        switch (true) {
            case $('#sourceUrl').val().length == 0:
                $('.cms2cms-drupal-error span').html('Url can`t be empty').parent().fadeIn();
                break;
            case !regex.test(sourceUrl):
                $('.cms2cms-drupal-error span').html('Url invalid, please enter url in "http://your_Drupal_website.com/" format').parent().fadeIn();
                break;
            case sourceUrl == targetUrl:
                $('.cms2cms-drupal-error span').html('Please, make sure to provide your source website URL (the URL of your current website)').parent().fadeIn();
                break;
            default :
                ...
                $.ajax({
                    ...
                    timeout: 1000,
                    success: function(){
                        ajaxValidate($('#verifySource_drupal').closest('form').serialize());
                    },
                    error: function(xhr) {
                        if (xhr.status == 200) {
                            ajaxValidate($('#verifySource_drupal').closest('form').serialize());
                        } else {
                            $('.cms2cms-drupal-error span').html('Please, provide a valid URL of your website.').parent().fadeIn()
                        }
                    }
                });
                break;
