...
/**
 * Class CmsPluginFunctionsDrupal
 */
class CmsPluginFunctionsDrupal
{

    const CMS2CMS_OPTION_TABLE = 'cms2cms_drupal_options';
    ...
    {
        $cms2cms_drupal_access_login = $this->getOption('cms2cms-drupal-login');
        $cms2cms_drupal_access_key = $this->getOption('cms2cms-drupal-key');

        return array(
            'email' => $cms2cms_drupal_access_login,
            'accessKey' => $cms2cms_drupal_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_drupal_access_key != false);
    }
    ...
    {
        $key = $this->getOption('cms2cms-drupal-key');
        $login = $this->getOption('cms2cms-drupal-login');
    ...
    public function saveOptions()
    {
        ...
        $cms2cms_drupal_site_url = $this->getSiteUrl();
        $bridge_depth = str_replace($cms2cms_drupal_site_url, '', $this->getFrontUrl());
        ...
        if ( $key && $login ) {
            $this->deleteOption('cms2cms-drupal-key');
            $this->setOption('cms2cms-drupal-key', $key);

            $this->deleteOption('cms2cms-drupal-login');
            $this->setOption('cms2cms-drupal-login', $login);

            $this->deleteOption('cms2cms-drupal-depth');
            $this->setOption('cms2cms-drupal-depth', $bridge_depth);
    ...
    {
        return $this->__('Drupal', 'cms2cms-drupal-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-drupal-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-drupal-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-drupal-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        ...
        if ($this->verifyFormTempKey($nonce, 'cms2cms_drupal_logout')
            && $_POST['cms2cms_drupal_logout'] == 1
        ) {
            $this->clearOptions();
        }
    }
